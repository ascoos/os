 ...
 * 
 * @desc <English> Demonstrates bulk file download using the bulkDownloadFiles method.
 * @desc <Greek> Παρουσιάζει τη μαζική λήψη αρχείων χρησιμοποιώντας τη μέθοδο bulkDownloadFiles.
 * 
 * @since PHP 8.2.0
 */
...
// <English> Define files to download
// <Greek> Ορισμός αρχείων για λήψη
$files = [
    '/remote/path/file1.txt' => $AOS_TMP_PATH . '/file1.txt',
    '/remote/path/file2.txt' => $AOS_TMP_PATH . '/file2.txt'
];

// <English> Define progress callback
// <Greek> Ορισμός callback για την πρόοδο
$progressCallback = function (array $progress) {
    echo "Downloaded {$progress['transferred']} of {$progress['total']} bytes for {$progress['file']}\n";
};

// <English> Bulk download files
// <Greek> Μαζική λήψη αρχείων
if ($ftpHandler->bulkDownloadFiles($files, FTP_BINARY, $progressCallback)) {
